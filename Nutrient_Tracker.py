    ...
    # foods list
        ...
        for i in lines:
            ...
            if splitLines[0] == item["product"]["product_name"]:
                ...
                if int(splitLines[1]) > 2:    # RATIONAL VARIABLE
...
def high_check(item):
    ...
    for i in FDA.readlines():
        ...
        try:
            ...
            if amount >= base * 0.15:    # RATIONAL VARIABLE
...
def check_past_foods():
    ...
    for i in lines:
        ...
        cutoffDate = pastDate[0:8] + str(int(pastDate[8:10]) + 2)  #RATIONAL
        # Check if we are passed the cutoff date                    VARIABLE
        if today >= cutoffDate:
...
# This function checks if you have eaten enough of the nutrients and if you
# are low on something, it will suggest what to eat based on your preferences
def assess_day():
    # Open up the FDA file to grab the nutrients
    FDA = open("FDA_DV.csv", 'r')
    # Then create a hashmap and populate it with the nutrients and the
    ...
    # Next we create another hashmap for the nutrients
    ...
    # We grab the food nutrient amounts from the today file and put it in the
    # hashmap for each food
    ...
    todayFoods.close()
    # Set flag values for future reminders
    user_vd, user_iron, user_calcium, user_potassium = 0, 0, 0, 0
    # Check if each value we have for consumed nutrients is up to
    # FDA standards
    ...
    # If every value is good we simply tell the user
    if user_vd == 0 and user_iron == 0 and user_calcium == 0 \
            and user_potassium == 0:
        print("Awesome! You have fulfilled all your nutrients "
              "requirement for today!")
    # If the user doesn't have enough of a certain nutrient we tell them
    if user_vd == -1:
        # We also calculate the amount they still need
        print("You still need {:.2f} mcg vitamin-d"
              .format(FDARecommended["vitamin-d"]
                      - todayFoodTracker["vitamin-d"]))
        # If the user is low on a food we recommend based on their frequently
        # eaten foods first
        frequentFoods = open("frequent_foods.csv", 'r')
        recommendedFood = None
        for eachFood in frequentFoods:
            # We check if the food is high in the specified nutrient
            food, nutrient = eachFood.strip().split(",")
            if nutrient == "vitamin-d":
                # If it is we assign it as the recommended food
                recommendedFood = food
                break
        # Remember to close the file
        frequentFoods.close()
        # if recommendedFood is None then search and recommend from
        # default_foods.csv using the same method as above
        if recommendedFood is None:
            ...
            for eachFood in defaultFoods:
                ...
                if nutrient == "vitamin-d":
                    ...
                    break
            defaultFoods.close()
        # Might be unnecessary but should still be present just in case
        if recommendedFood is None:
            print("Something is missing!  Please try again")
        print("{:s} is rich in vitamin-d.  Try eating this to raise your "
              "amount and reach your vitamin-d goal!".format(recommendedFood))
        print()

    # These next three sections do the exact same thing for each other nutrient
    if user_iron == -1:
        print("You still need {:.2f} mg iron"
              .format(FDARecommended["iron"]-todayFoodTracker["iron"]))
        # Recommend iron food
        frequentFoods = open("frequent_foods.csv", 'r')
        recommendedFood = None
        for eachFood in frequentFoods:
        ...
        if recommendedFood is None:
            ...
            for eachFood in defaultFoods:
                ...
                if nutrient == "iron":
                    ...
                    break
            defaultFoods.close()
        print("{:s} is rich in iron.  Try eating this to raise your amount and"
              " reach your iron goal!".format(recommendedFood))
        print()

    if user_calcium == -1:
        print("You still need {:.2f} mg calcium"
              .format(FDARecommended["calcium"]
                      - todayFoodTracker["calcium"]))
        ...
        if recommendedFood is None:
            ...
            for eachFood in defaultFoods:
                ...
                if nutrient == "calcium":
                    ...
                    break
            defaultFoods.close()
        print("{:s} is rich in calcium.  Try eating this to raise your "
              "amount and reach your calcium goal".format(recommendedFood))
        print()

    # potassium check
    if user_potassium == -1:
        print("You still need {:.2f} mg potassium"
              .format(FDARecommended["potassium"]
                      - todayFoodTracker["potassium"]))
        ...
        if recommendedFood is None:
            ...
            for eachFood in defaultFoods:
                ...
                if nutrient == "potassium":
                    ...
                    break
            defaultFoods.close()
        print("{:s} is rich in potassium.  Tray eating this to raise your"
              " amount and reach your calcium goal!".format(recommendedFood))
        print()


# HELPFUL BARCODES
# 038000169663 pringles - No trigger
# 719283995391 Meijer hot chocolate - Calcium Trigger
# 029000000360 Planters Peanuts - Iron Trigger
main()
